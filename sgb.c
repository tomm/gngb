...
inline void sgb_line_mode(void) {
  ...
  while(nb_dataset>0 && i<SGB_PACKSIZE) {
    ...
    if (sgb.pack[i]&0x40)  // mode vertical
      set_pal_Vline(sgb.pack[i]&0x1f,(sgb.pack[i]&0x60)>>5);
    else  // mode horizontal
      set_pal_Hline(sgb.pack[i]&0x1f,(sgb.pack[i]&0x60)>>5);
    i++;
  }
...
inline void sgb_divide_mode(void) {
  ...
  if (!(sgb.pack[1]&0x40)) { 
    ...
    /* Left of the line */
    //set_pal_Vline_range(0,line-1,(sgb.pack[1]&0xc0)>>2);
    set_pal_Vline_range(0,line-1,sgb.pack[1]&0x03);
    /* Right of the line */
    //set_pal_Vline_range(line+1,17,sgb.pack[1]&0x03);
    set_pal_Vline_range(line+1,19,(sgb.pack[1]&0x0c)>>2);
  } else { 
    /* On the line */
    set_pal_Hline(line,(sgb.pack[1]&0x30)>>4);
    /* Above of the line */
    set_pal_Hline_range(0,line-1,sgb.pack[1]&0x03);
    //set_pal_Hline_range(0,line-1,(sgb.pack[1]&0xc0)>>2);
    /* Below of the line */
    set_pal_Hline_range(line+1,17,(sgb.pack[1]&0x0c)>>2);
    //set_pal_Hline_range(line+1,19,sgb.pack[1]&0x03);
  }
  SGB_CMD_END();
}
...
inline void sgb_1chr_mode(void) {
  ...
  if (sgb.nb_pack==-1) {  // first call
    ...
    nb_dataset=(sgb.pack[4]<<8)|sgb.pack[3];
...
void sgb_exec_cmd(void) {
  ...
  if (sgb.cmd==0xff) {
    /*int i;
      printf("LCDCONT: %02x sgb:%02x nb:%d pack: ",LCDCCONT,sgb.pack[0]>>3,sgb.pack[0]&0x07);
